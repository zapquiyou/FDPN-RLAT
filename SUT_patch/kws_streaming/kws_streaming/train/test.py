...
def tf_non_stream_model_accuracy(
  ...
  set_size = audio_processor.set_size(test_set)
  ...
  result_dic = {'result': [], 'path': []}
  for i in range(0, set_size, flags.batch_size):
    test_fingerprints, test_ground_truth = audio_processor.get_data(
        flags.batch_size, i, flags, 0.0, 0.0, time_shift_samples, test_set,
        0.0, 0.0, sess)
    ...
    # Record per-sample inference correctness and corresponding file path.
    correct_mask = (predicted_labels == test_ground_truth)
    for j, is_correct in enumerate(correct_mask):
      result_dic['result'].append(bool(is_correct))
      result_dic['path'].append(audio_processor.data_index[test_set][i + j]['file'])
  ...
  if result_file_path:
    ...
    with open(result_file_path, 'wt') as fd:
      json.dump(result_dic, fd, ensure_ascii=False)
